...
export async function sendEmail(params: SendEmailParams): Promise<{ sent: boolean; error?: string }> {
  const log = await db.emailLog.create({
    data: {
      to:       params.to,
      from:     process.env.EMAIL_FROM || 'noreply@holaprime.com',
  ...
  try {
    ...
    if (!transporter) {
      const err = 'No email provider configured. Go to Admin → Email → SMTP Config and save your settings.'
      if (log) await db.emailLog.update({ where: { id: log.id }, data: { status: 'FAILED', error: err } }).catch(() => {})
    ...
    if (log) await db.emailLog.update({ where: { id: log.id }, data: { status: 'SENT', sentAt: new Date() } }).catch(() => {})
    return { sent: true }
  } catch (error: any) {
...
// Keep boolean wrapper for backward compatibility
export async function sendEmailBool(params: SendEmailParams): Promise<boolean> {
...
export function templateRegistrationConfirm(params: {
  firstName: string; displayName: string; countryName: string
}) {
...
export function templateAdminInvite(params: {
  ...
  .warn{background:rgba(240,192,64,0.08);border:1px solid rgba(240,192,64,0.2);border-radius:8px;padding:12px;font-size:13px;color:rgba(240,192,64,0.9);margin-top:16px}
</style></head><body>
  <div class="container">
    <div class="card">
      <h2>Welcome, ${params.firstName}! 🛡</h2>
      <p>You have been granted <strong>${roleLabel}</strong> access to the Hola Prime World Cup admin panel by <strong style="color:#00d4ff">${params.invitedBy}</strong>.</p>
      <div class="cred">
        <div><span style="color:rgba(180,200,235,0.6);font-size:12px">Login URL</span><br><span style="color:#00d4ff">${params.loginUrl}</span></div><br>
        <div><span style="color:rgba(180,200,235,0.6);font-size:12px">Email</span><br><span style="color:#00d4ff">${params.email}</span></div><br>
        <div><span style="color:rgba(180,200,235,0.6);font-size:12px">Temporary Password</span><br><span style="color:#00d4ff">${params.password}</span></div><br>
        <div><span style="color:rgba(180,200,235,0.6);font-size:12px">Role</span><br><span style="color:#00d4ff">${roleLabel}</span></div>
      </div>
      <div class="warn">⚠ Please log in and change your password immediately. Do not share these credentials.</div>
      <div style="text-align:center"><a href="${params.loginUrl}" class="btn">Access Admin Panel →</a></div>
    </div>
  </div>
</body></html>`
}
